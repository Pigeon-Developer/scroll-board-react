...
import { randomColor, randomInt, sleep } from "./util";

const MockTeamLength = 80;
...
export function prepareMockData() {
  ...
  const initList = new Array(MockTeamLength).fill(0).map((_, index) => {
    return {
      id: index,
      bg: randomColor(),
      rank: -1,
...
async function makeRandomData(startMs: number) {
  const teamId = randomInt(MockTeamLength);
